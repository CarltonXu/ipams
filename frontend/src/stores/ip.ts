...
export const useIPStore = defineStore('ip', {
  ...
  actions: {
    ...
    async fetchAllIPs() {
      this.loading = true;
      try {
        const response = await axios.get(`${API_CONFIG.BASE_API_URL}${API_CONFIG.ENDPOINTS.IP.LIST}`); // 不带分页参数
    ...
    async fetchPaginatedIPs(page: number, pageSize: number) {
      ...
      try {
        const response = await axios.get('`${API_CONFIG.BASE_API_URL}${API_CONFIG.ENDPOINTS.IP.LIST}`', {
    ...
    async updateIP(ipId: string, data: {
      ...
      try {
        await axios.post(`${API_CONFIG.BASE_API_URL}${API_CONFIG.ENDPOINTS.IP.UPDATE(ipId)}`, data);
        return true;
      } catch (err: any) {
    ...
    async batchClaimIPs(ipConfigs: Array<{
      ...
      try {
        await Promise.all(ipConfigs.map(config => 
          axios.post(`${API_CONFIG.BASE_API_URL}${API_CONFIG.ENDPOINTS.IP.CLAIM(config.id)}`, {
    ...
    async batchUpdateIPs(ips: string[], data: {
      ...
      try {
        await Promise.all(ips.map(ipId => 
          axios.post(`${API_CONFIG.BASE_API_URL}${API_CONFIG.ENDPOINTS.IP.UPDATE(ipId)}`, data)
        ));
        return true;
      } catch (err: any) {
        this.error = err.message;
