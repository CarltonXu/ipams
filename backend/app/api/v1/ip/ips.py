...
@ips_bp.route('/ip/<ip_id>/claim', methods=['POST'])
@token_required
def claim_ip(current_user, ip_id):
    ...
    # 发送通知
    send_notification(
        event=NotificationEvent.IP_CLAIMED,
        user=current_user,
        template_data={
            'ip': ip.ip_address,
...
@ips_bp.route('/ip/<ip_id>', methods=['POST'])
@token_required
def update_ip(current_user, ip_id):
    ...
    validation_error = helpers.validate_update_data(data)
    if validation_error:
