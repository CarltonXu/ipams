...
def upload_avatar(current_user):
    ...
    if not helpers.allowed_file(file.filename, {'png', 'jpg', 'jpeg', 'gif'}):
    ...
    for field in allowed_fields:
        if field in data:
            ...
            if field == 'email' and data[field] != user.email:
                if not helpers.is_valid_email(data[field]):
                    return jsonify({'error': 'Invalid email format'}), 400
...
@user_bp.route('/user/check-ips', methods=['POST'])
@token_required
...
@user_bp.route('/user/batch-delete', methods=['POST'])
@token_required
@admin_required
